...
public:
    ...
    bool _spi_setMAC(std::vector<uint8_t> & mac);
    bool _spi_setMAC(u8t *mac);
    bool _spi_setMAC(const char* mac);
    bool _spi_setMAC(macaddr_t mac);
...
private:
    ...
    void _spi_readBuffMemory(std::vector<uint8_t>&buff, size_t size);
    void _spi_writeBuffMemory(std::vector<uint8_t>&buff);

    void _spi_bitFieldSet(uint8_t reg, uint8_t data);
